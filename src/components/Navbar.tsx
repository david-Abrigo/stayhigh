...
import { ShieldCheck, Zap, ArrowUpRight, Menu, X, Code2, Tag, Home, Play } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ currentPath = '/', onNavigate, onReset }) => {
    ...
    <header className="pt-4 px-4 sm:px-6 max-w-5xl mx-auto w-full relative z-50">
      <div className="bg-white/90 backdrop-blur-md border border-white/70 shadow-[0_4px_25px_rgba(0,0,0,0.03)] rounded-2xl px-4 sm:px-6 h-16 sm:h-18 flex items-center justify-between transition">
        ...
        <button
          type="button"
          onClick={() => handleNav('/')}
          className="flex items-center gap-3 text-left focus:outline-none focus:ring-2 focus:ring-brand-mint rounded-xl transition group cursor-pointer"
        >
